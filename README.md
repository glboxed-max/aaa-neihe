...
| 想要的效果 | 选择 | 目标数 |
|---|---|---|
| 每版本最新（默认） | `全部版本` + `最新子版本` | 5 |
| 全部版本 × 全部子版本 | `全部版本` + `全部子版本` | 96 |
| 5 系列全部子版本 | `5系列` + `全部子版本` | 47 |
| 5 系列最新 | `5系列` + `最新子版本` | 2 |
| 6 系列全部子版本 | `6系列` + `全部子版本` | 49 |
| 6 系列最新 | `6系列` + `最新子版本` | 3 |
| 只构建一个 | `单独指定` + 填 5 个输入 | 1 |

### 版本表（`versions.tsv`）
格式：`android|kernel|sub_level|os_patch_level|revision`，`#` 开头为注释，`X|lts` 表示 lts 分支（“最新子版本”模式自动忽略）。范围对齐 ABK：
