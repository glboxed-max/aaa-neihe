#!/bin/bash
# KernelSU 非 GKI「手动 hook」补丁（在设备内核源码根目录执行）
#
# 手动 hook 由 KernelSU 上游提供（weishu <twosxtd@gmail.com>, GPL-3.0），
# 这里整理为脚本以便在 4.x / 5.4 等老内核上一键打点。
# kprobes 在 4.x 上不可靠，因此老内核推荐改用手动 hook。
#
# 覆盖文件：fs/exec.c fs/open.c fs/read_write.c fs/stat.c drivers/input/input.c
# 已存在于源码的 ksu 调用会被跳过（幂等）。
...
for i in "${patch_files[@]}"; do
    [ -f "$i" ] || { echo "[-] 跳过不存在: $i"; continue; }

    if grep -q "ksu" "$i"; then
        echo "[=] $i 已含 KernelSU，跳过"
        continue
    fi
